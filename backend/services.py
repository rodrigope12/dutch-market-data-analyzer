...
class ComplianceService:
    ...
    def process_invoice(self, invoice: Invoice) -> ProcessingResult:
        ...
        if critical_failures:
            ...
            risk_score = 100
            logger.info(f"Invoice {invoice.invoice_id} rejected. Failures: {len(critical_failures)}")
        elif warnings:
            final_status = "DRAFT" # Review required
            risk_score = 50
            logger.info(f"Invoice {invoice.invoice_id} flagged for review.")
        else:
            final_status = "APPROVED"
            risk_score = 0
            
            # Action: Post to Odoo if Approved
            self._post_to_odoo_mock(invoice)
            logger.info(f"Invoice {invoice.invoice_id} approved and posted to Odoo.")
    ...
    def _post_to_odoo_mock(self, invoice: Invoice) -> bool:
        """
        Mockup of an API call to Odoo ERP.
        In production, this would use xmlrpc to post to 'account.move'.
        """
        logger.info(f"[ODOO API] > POST /api/v1/account.move")
        logger.info(f"[ODOO API] > Payload: {{ 'partner_id': '{invoice.vendor_name}', 'amount': {invoice.amount}, 'ref': '{invoice.invoice_id}' }}")
        logger.info(f"[ODOO API] < 201 Created (ID: {int(time.time())})")
        return True
    ...
    def _validate_budgetary_alignment(self, invoice: Invoice) -> CheckResult:
        ...
        if invoice.department == "Unknown":
            ...
            if invoice.amount > remaining:
                ...
                msg = f"Funds available ({remaining - invoice.amount} remaining)"
    ...
    def _verify_contractual_standing(self, invoice: Invoice) -> CheckResult:
        ...
        for _, agreement in agreements.iterrows():
            if agreement['is_active'] and agreement['start_date'] <= inv_date_iso <= agreement['end_date']:
                status = CheckStatus.PASS
                msg = f"Covered by contract {agreement.get('id', '')}."
                break
